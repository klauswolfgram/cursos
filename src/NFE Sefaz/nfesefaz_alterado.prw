...
/* PONTO DE ENTRADA PARA TRATAMENTO SEGMENTO GRANITO */

If ExistBlock("PEGRNFESEFAZ")

	_aNfAux := aclone(aNfVinc)
...
//	aNfVinc    := aParam[11]
	...
	aNfVinc	   := getnfvinc()
	lVinc	   := iif(empty(aNfVinc),.F.,.T.)			

EndIf

/* FIM PONTO DE ENTRADA PARA TRATAMENTO SEGMENTO GRANITO */
...
//-- Klaus, 16/07/2021 - comentada a condicao padrao e incluida nova condicao so com aNfVinc
If .not. empty(aNfVinc) //(!Empty(aNfVinc)	.And. Empty(aExp[1])) .or.(!Empty(aNfVinc).And. !Empty(aExp[1]) .and. lEECFAT)
	cString += '<NFRef>'
			...
			*/
			If (valType(aPPDifal)== "A" .and. Len(aPPDifal)>0 .and. Year(aNota[03]) >= aPPDifal[1][1])
				...
				If Alltrim(Str(aProd[33])) == "4" .Or. Alltrim(Str(aProd[33])) == "7" .Or. Alltrim(Str(aProd[33])) == "12"
					...
					cString += '<ValorICMSRem>'+ConvType(0,15,2)+'</ValorICMSRem>' //vICMSUFReme	
					cString += '</Tributo>'
					cString += '</imposto>'					
...
Static Function getnfvinc
	...
	_cAliasSQL := getnextalias()

	BeginSQL Alias _cAliasSQL
		COLUMN F2_EMISSAO AS DATE
		SELECT F2_EMISSAO, F2_SERIE, F2_DOC, F2_FILIAL, F2_ESPECIE, F2_CHVNFE 
		FROM %Table:CDD% CDD
		JOIN %Table:SF2% SF2 ON SF2.%notdel% AND F2_CHVNFE = CDD_CHVNFE
		WHERE CDD.%notdel%
		AND CDD_FILIAL 	= %Exp:xFilial('CDD')%
		AND CDD_DOC 	= %Exp:SF2->F2_DOC%
		AND CDD_SERIE 	= %Exp:SF2->F2_SERIE%
	EndSQL		

	While .not. (_cAliasSQL)->(eof())

		aadd(aNfAux,{(_cAliasSQL)->F2_EMISSAO,;
			(_cAliasSQL)->F2_SERIE  		 ,;
			(_cAliasSQL)->F2_DOC    		 ,;
			SM0->M0_CGC			  		     ,;
			SM0->M0_ESTCOB		    		 ,;
			(_cAliasSQL)->F2_ESPECIE		 ,;
			(_cAliasSQL)->F2_CHVNFE			 ,0,"","",0,"",""})
	...
	For x := 1 To Len(aNfAux)

		_cChaveNFE := aNfAux[x,7]

		IF .not. _cChaveNFE $ cMensCli

			IF .not. "Chave de acesso NF-E referenciada:" $ cMensCli
				cMensCli += " Chave de acesso NF-E referenciada: "
			EndIF

			cMensCli += ' ' + _cChaveNFE + ' ' + CRLF
